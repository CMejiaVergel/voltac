...
import { crearProspecto } from "./actions";
...
export default function CotizarPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    /*
      Se comprueba el resultado, no solo que no lance. Un formulario que celebra
      el envio sin mirar la respuesta pierde prospectos en silencio, y eso ya
      nos costo caro en la linea de energia.
    */
    try {
      const res = await crearProspecto(data);
      if (res.ok) setSuccess(true);
      else setError(res.error);
    } catch (err) {
      console.error(err);
      setError("No pudimos contactar al servidor. Revise su conexión e inténtelo de nuevo.");
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              >
                ...
                <p className="lead text-secondary/70">
                  Cuéntenos qué equipo lo tiene preocupado, qué lazo terminó operándose en manual
                  o qué parada se les alargó la última vez. Vamos, lo miramos y le decimos con
                  franqueza si hay algo que valga la pena intervenir, qué costaría y qué recupera.
              ...
              <RevealGroup className="space-y-6" stagger={0.08}>
                <RevealItem className="flex items-center gap-4 bg-white p-4 rounded-xl border border-border shadow-sm hover:border-primary/30 hover:shadow-md transition-all duration-300">
                   ...
                   <button onClick={copyEmail} className="text-secondary/50 hover:text-primary transition-colors p-2" aria-label="Copiar correo">
                       {copiedData ? <span className="text-xs text-primary-ink font-bold">Copiado</span> : <Copy size={20} />}
                   </button>
                </RevealItem>
                ...
                {success ? (
                  <div className="relative z-10 py-12 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-secondary/70 font-light mb-8 max-w-sm leading-relaxed">
                      Nos comunicamos con usted en las próximas horas hábiles para coordinar la
                      visita técnica. Si prefiere adelantarla, escríbanos a{" "}
                    ...
                    <form className="space-y-6 relative z-10" onSubmit={handleSubmit}>
                      ...
                      {error && (
                        <p className="text-destructive text-sm font-medium bg-destructive/10 border border-destructive/20 rounded-xl px-4 py-3">
                          {error}{" "}
                          <a href={CONTACT.whatsapp} target="_blank" rel="noopener noreferrer" className="underline font-semibold">
                      ...
                      <div className="pt-4">
                        <Button type="submit" disabled={loading} variant="default" size="lg" className="w-full h-14 text-base tracking-wider rounded-xl hover:shadow-[0_0_20px_rgba(242, 101, 17,0.4)] transition-all">
                            {loading ? "Enviando..." : "Solicitar la visita técnica"}
                        </Button>
