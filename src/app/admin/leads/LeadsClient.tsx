...
export default function LeadsClient({ initialLeads }: { initialLeads: any[] }) {
  ...
  const proposalLeads = activeLeads.filter(l => l.stage === 'Cotización Enviada');
  ...
  const filteredLeads = leads.filter(l => {
    ...
    const matchesSearch = search ? 
      [l.fullName, l.email, l.phone, l.company].some(val => val?.toString().toLowerCase().includes(search.toLowerCase())) 
      : true;
    ...
    return matchesSearch && matchesStage && matchesStatus && matchesPriority && matchesProject && matchesSource && matchesOverdue && matchesUncontacted;
  });
  ...
  const handleExportCSV = () => {
    const headers = ["ID", "Fecha", "Nombre", "Empresa", "Celular", "Correo", "Presupuesto", "Etapa", "Estado", "Prioridad", "Requerimiento", "Fuente"];
    ...
    link.href = url;
    link.setAttribute("download", `Leads_Sistemas_${new Date().getTime()}.csv`);
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-secondary/60">Gestiona prospectos tecnológicos, agentes IA y aplicaciones B2B.</p>
        </div>
        ...
        <div className="p-4 border-b border-border flex flex-col lg:flex-row gap-4 items-center justify-between bg-muted/30">
          <div className="flex flex-wrap items-center gap-3 w-full">
            ...
            <select className="px-3 py-2 border border-border rounded-xl text-sm bg-white outline-none cursor-pointer text-secondary/70 shrink-0"
              ...
              <option value="Todos">Tipo de Lead</option>
              <option value="Desarrollo Web">Desarrollo Web</option>
              <option value="IA / Agentes">IA / Agentes</option>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-secondary/5 text-secondary/60 uppercase tracking-widest text-[#10px] sm:text-xs font-bold border-b border-border">
              <tr>
                ...
                <th className="px-6 py-4">Empresa / Proyecto</th>
            ...
            <tbody className="divide-y divide-border">
              {filteredLeads.map((lead: any) => {
                ...
                const isUrgentNew = lead.stage === 'Nuevo Prospecto' && (new Date().getTime() - new Date(lead.createdAt).getTime() > 7*24*60*60*1000);
                  ...
                  <tr key={lead.id} className={`hover:bg-muted/30 transition-colors group cursor-pointer ${selectedIds.includes(lead.id) ? 'bg-primary/5' : ''}`} onClick={() => setActiveLead(lead)}>
                    ...
                    <td className="px-6 py-4" onClick={e => e.stopPropagation()}>
                       ...
                       >
                         ...
                         <option value="Ganado">✓ Ganado / Proyecto en Marcha</option>
                         <option value="Perdido">✗ Perdido</option>
                       </select>
