...
export default async function AdminDashboardPage() {
  ...
  const techTypes = new Set(publishedProjects.map((p: any) => p.techType));
  const techDiversity = techTypes.size.toString() + ' Stacks';
  ...
  const metrics = [
    { label: "Leads Activos", value: activeLeadsCount.toString(), change: "Oportunidades de Negocio", icon: <Users size={24} className="text-blue-500" /> },
    { label: "Proyectos Publicados", value: publishedProjects.length.toString(), change: "Portafolio en Línea", icon: <FolderKanban size={24} className="text-orange-500" /> },
    { label: "Diversidad Stack", value: techDiversity, change: "Tecnologías Dominadas", icon: <Zap size={24} className="text-yellow-500" /> },
    { label: "Blog Técnico", value: news.length.toString(), change: "Entradas Públicas", icon: <Newspaper size={24} className="text-accent" /> },
  ];
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-secondary/60">Bienvenido al panel central de Voltac Systems.</p>
      </div>
               ...
               recentLeads.map((lead: any) => (
                 <Link href={`/admin/leads?id=${lead.id}`} key={lead.id} className="flex justify-between items-center p-4 bg-muted/30 rounded-xl border border-border hover:bg-muted transition-colors">
                   <div>
                     ...
                     <p className="text-[10px] text-secondary/60 mt-1 capitalize">{lead.source}</p>
                   </div>
             ...
             {recentProjects.length === 0 ? (
               ...
               recentProjects.map((p:any) => (
                 <Link href="/admin/proyectos" key={p.id} className="flex gap-4 items-center p-4 bg-muted/30 rounded-xl border border-border hover:bg-muted transition-colors">
                   ...
                   <div>
                     <p className="font-bold text-sm text-secondary line-clamp-1">{p.title || 'Sin título'}</p>
                     <p className="text-[10px] uppercase tracking-widest text-secondary/60 font-bold">{p.techType}</p>
                   </div>
                 </Link>
               ))
             )}
