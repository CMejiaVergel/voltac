...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: `${entry.titulo} | Voltac Systems`,
      ...
      <div className="container mx-auto px-4 md:px-6 py-16">
         <div className="grid lg:grid-cols-12 gap-12 lg:gap-16">
            ...
            <aside className="lg:col-span-4 space-y-8">
               <div className="bg-white rounded-[2rem] border border-border/50 shadow-lg p-6 md:p-8 sticky top-28">
                  ...
                  {relatedNews.length === 0 ? (
                     <p className="text-sm font-light text-secondary/50">Por ahora no hay más noticias publicadas.</p>
                  ) : (
                           ...
                           <Link key={rn.id} href={`/noticias/${rn.slug}`} className="group flex flex-col gap-3">
                              ...
                              <div>
                                 ...
                                 <p className="text-[10px] uppercase tracking-widest text-secondary/50 mt-1 font-bold">
                                    {new Date(rn.fecha_publicacion).toLocaleDateString('es-CO')}
                                 </p>
                              </div>
                           </Link>
                        ))}
